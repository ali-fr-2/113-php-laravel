...
if (isset($_POST['submit'])) {
    ...
    if (empty($introduction_file)) {
        $$introduction_file = $course['video'];
    } else {
    ...
    $result = $conn->prepare("UPDATE courses SET title=?, caption=?, image=?,	introduction=?,	price=?, category=?,	level=?,	status=?, tag=?,teacher=?, create_date=? ");
                ...
                <form id="upload-form" action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <label for="name" class="text-gray-600 fw-bold">
                                قیمت دوره</label>
                            <input name="price" id="name" type="text" class="form-control mt-2" value="<?= $course['price']; ?>" />
                        </div>
                    </div>
                    <br>
                    <div class="row">
